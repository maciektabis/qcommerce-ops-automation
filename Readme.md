...
- **Auto-Shift Bridging (Łamanki):** An automated DOM-manipulation script that clicks through the interface to automatically close and split overlapping shifts. **It turns a tedious a few minutes clicking process into a 5-second automated task.**
...
- **Auto-Łamanki (Łączenie zmian):** Zautomatyzowany skrypt manipulujący modelem DOM, który przeklikuje się przez interfejs, aby automatycznie zamykać i rozdzielać nakładające się zmiany. **Zamienia kilku-minutowy, frustrujący proces ręczny w 5-sekundowe kliknięcie.**
